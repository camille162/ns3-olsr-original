                      ...
                      Time stopTime)
{
    ...
    if (recoveryStats)
    {
        if (!recoveryStats->inOutage && connectivityPct < recoveryDropThresholdPct)
        {
            recoveryStats->inOutage = true;
            recoveryStats->outageStartSec = nowS;
            std::cout << "RECOVERY_EVENT type=drop startSec=" << nowS
                      << " connectivityPct=" << connectivityPct << std::endl;
        }
        else if (recoveryStats->inOutage && connectivityPct >= recoveryRestoreThresholdPct)
        {
            ...
            std::cout << "RECOVERY_EVENT type=restore timeSec=" << nowS
                      << " recoverySec=" << recoverySec
                      << " connectivityPct=" << connectivityPct << std::endl;
        }
    }
    std::cout << "TIME " << nowS
              << " METRICS connectivityPct=" << connectivityPct
              << " goodputRatioPct=" << goodputRatioPct
              << " avgDelayMs=" << avgDelayMs
              << " totalThroughputKbps=" << connectedThroughputKbps
              << " connectedFlows=" << connected
              << " activeFlows=" << active
              << " totalFlows=" << total << std::endl;
...
main(int argc, char* argv[])
{
    ...
    double recoveryDropThresholdPct = 50.0;
    double recoveryRestoreThresholdPct = 80.0;
    ...
    cmd.AddValue("recoveryDropThresholdPct",
                 "Outage starts when connectivityPct drops below this threshold",
                 recoveryDropThresholdPct);
    cmd.AddValue("recoveryRestoreThresholdPct",
                 "Recovery ends when connectivityPct reaches this threshold",
                 recoveryRestoreThresholdPct);

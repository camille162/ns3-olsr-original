...
///
/// @brief ETX-OLSR routing protocol implementation.
///
/// This file follows the structure of olsr-routing-protocol.cc from the
/// ns-3 OLSR module.  All RFC 3626 control-plane behaviour is preserved;
/// the only functional difference is the route-selection metric.
///
...
// ---- Link / Neighbor type enumerations (local to this TU) ----
...
inline std::ostream&
operator<<(std::ostream& os, LinkType lt)
{
    switch (lt)
    {
...
inline std::ostream&
operator<<(std::ostream& os, NeighborType nt)
{
    switch (nt)
    {
    ...
    static TypeId tid =
        TypeId("ns3::etxolsr::RoutingProtocol")
            ...
            .AddAttribute("Willingness",
                          "Willingness to carry traffic for other nodes.",
            ...
            .AddAttribute("EtxAlpha",
                          "EWMA smoothing factor for ETX estimation (0 = no smoothing, "
                          "1 = never update). Recommended range: 0.6 - 0.9.",
                          DoubleValue(0.7),
                          MakeDoubleAccessor(&RoutingProtocol::m_etxAlpha),
                          MakeDoubleChecker<double>(0.0, 1.0))
            .AddAttribute("InitialEtx",
                          "Default ETX value used for links whose quality is not yet known.",
                          DoubleValue(3.0),
...
RoutingProtocol::UpdateNeighborEtx(const Ipv4Address& neighborIfaceAddr, Time helloInterval)
{
    ...
    if (it == m_etxMap.end())
    {
        // First HELLO from this neighbor: initialise EWMA with one good sample.
        EtxInfo info;
        info.prr = (1.0 - m_etxAlpha); // alpha*0 + (1-alpha)*1
    ...
    // Determine hello interval to use (prefer what the sender advertised).
    ...
    // Estimate how many HELLO intervals have passed since the last one we received.
    ...
    // Apply missed hellos (PRR sample = 0 for each).
    for (int i = 0; i < missed; i++)
    {
        info.prr = m_etxAlpha * info.prr;
    }

    // Apply this received hello (PRR sample = 1).
    ...
    for (auto messageIter = messages.begin(); messageIter != messages.end(); messageIter++)
    {
        ...
        if (duplicated == nullptr)
        {
            ...
            case olsr::MessageHeader::HELLO_MESSAGE:
                ...
                // Update ETX estimate BEFORE standard hello processing so that
                // the neighbor PRR is already updated when RoutingTableComputation runs.
                UpdateNeighborEtx(senderIfaceAddr,
                                  messageHeader.GetHello().GetHTime());
...
// ---- MprComputation (identical to OLSR) ----

namespace
{
/**
 * Remove all covered 2-hop neighbors belonging to nb_tuple from N2.
 */
void
CoverTwoHopNeighbors(Ipv4Address neighborMainAddr, olsr::TwoHopNeighborSet& N2)
{
    // first gather all 2-hop neighbors of nb_tuple
...
RoutingProtocol::MprComputation()
{
    ...
    // 1. start with ALWAYS-willing neighbors
    ...
    // 2. D(y) = degree of coverage of each 2-hop neighbor
    int stage = 1;
    while (true)
    {
        // (a) remove all 2-hop neighbors already reachable via MPR
        ...
        if (stage == 1)
        {
            // Select MPRs that are the *only* node able to cover some 2-hop neighbors.
            for (auto nb2hop_tuple = N2.begin(); nb2hop_tuple != N2.end(); nb2hop_tuple++)
            {
                NS_LOG_LOGIC("Considering 2-hop neighbor: "
                             << nb2hop_tuple->twoHopNeighborAddr
                             << " via " << nb2hop_tuple->neighborMainAddr);
                ...
                for (auto nb_tuple = N.begin(); nb_tuple != N.end(); nb_tuple++)
                {
                    ...
                    for (auto nb2hop_tmp = N2.begin(); nb2hop_tmp != N2.end(); nb2hop_tmp++)
                    {
                        if (nb2hop_tmp->neighborMainAddr == nb_tuple->neighborMainAddr &&
                            nb2hop_tmp->twoHopNeighborAddr == nb2hop_tuple->twoHopNeighborAddr)
                        {
                            canCover = true;
                            break;
                        }
                ...
                int reachability = 0;
                std::set<Ipv4Address> coveredByOnlyOne;
                for (auto nb_tuple = N.begin(); nb_tuple != N.end(); nb_tuple++)
                {
                    if (nb_tuple->status != olsr::NeighborTuple::STATUS_SYM ||
                ...
                (void)reachability;
                (void)canCover;
                (void)coveredByOnlyOne;
            }
            stage = 2;
        }
        else
        {
            // Stage 2: select 1-hop neighbor covering most 2-hop neighbors.
            Ipv4Address maxAddr;
...
// ---- RoutingTableComputation (ETX-based) ----
...
RoutingProtocol::RoutingTableComputation()
{
        ...
        if (nb_tuple.status == olsr::NeighborTuple::STATUS_SYM)
        {
            ...
            for (auto it2 = linkSet.begin(); it2 != linkSet.end(); it2++)
            {
                    ...
                    link_tuple.time >= Simulator::Now())
                {
                    ...
                    // Keep entry only if it has a better (lower) ETX.
            ...
            // Ensure main address has a route entry.
    ...
    for (auto it = twoHopNeighbors.begin(); it != twoHopNeighbors.end(); it++)
    {
        ...
        {
            NS_LOG_LOGIC("Two-hop neighbor is also a 1-hop neighbor; skipped.");
            continue;
        }
        ...
        {
            // Cost for the 1-hop→2-hop link is unknown; use InitialEtx.
    ...
    // 4. Multi-hop routes via topology tuples (Bellman-Ford style with ETX cost).
    //    Run until no improvement is found.
    bool improved = true;
    while (improved)
    {
        improved = false;
        ...
        for (auto it = topology.begin(); it != topology.end(); it++)
        {
            ...
            NS_LOG_LOGIC("Topology tuple: " << topology_tuple);
            ...
            // Use InitialEtx as the per-hop cost for links we cannot directly measure.
            ...
            {
                NS_LOG_LOGIC("Adding/updating route to " << topology_tuple.destAddr
                                                         << " via " << topology_tuple.lastAddr
                                                         << " etx=" << newEtx
                                                         << " hops=" << newDist);
    ...
    // 5. Interface association entries (same as standard OLSR).
    ...
    // 6. HNA routes (same as standard OLSR).
                               ...
                               const Ipv4Address& senderIface)
{
...
#ifdef NS3_LOG_ENABLE
    {
        ...
        NS_LOG_DEBUG(Simulator::Now().As(Time::S)
                     << " ** BEGIN dump Link Set for ETX-OLSR Node " << m_mainAddress);
        for (auto link = links.begin(); link != links.end(); link++)
        {
            NS_LOG_DEBUG(*link);
        }
        NS_LOG_DEBUG("** END dump Link Set for ETX-OLSR Node " << m_mainAddress);
    }
#endif
...
// ---- QueueMessage / SendPacket / SendQueuedMessages ----
...
// ---- Duplicate / Link / Neighbor / etc tuple management ----
                              ...
                              Socket::SocketErrno& sockerr)
{
    ...
    if (Lookup(header.GetDestination(), entry1))
    {
        ...
        NS_LOG_DEBUG("ETX-OLSR node " << m_mainAddress
                                      << ": RouteOutput for dest=" << header.GetDestination()
                                      << " --> nextHop=" << entry2.nextAddr
                                      << " etx=" << entry1.etxDistance);
        found = true;
    }
    else
    {

 ...
 */
 ...
 #include "etx-olsr-msg-header.h"
 #include "etx-olsr-repositories.h"
 #include "etx-olsr-state.h"
 ...
 private:
   ...
   double m_etxAlpha;
   double m_initialEtx;
   /// When true, append ETX-OLSR TC trailer (same UDP datagram; not in OLSR packet length).
   bool m_enableTcLinkEtx;
   /// Parallel to queued OLSR messages: optional TC ETX sidecar for serialization.
   std::deque<std::optional<TcEtxTrailerBlock>> m_queuedTcEtxExtensions;
   /// Trailer blocks received with the current OLSR packet (consumed before message dispatch).
   std::vector<TcEtxTrailerBlock> m_rxTcEtxTrailerThisPkt;
   struct AdvertisedEdgeEtx
   {
     double etx;
     Time expire;
   };
   std::map<std::pair<Ipv4Address, Ipv4Address>, AdvertisedEdgeEtx> m_linkEtxByTcAdvert;
   /// TC originator + message sequence → last received trailer block (for MPR retransmit).
   std::map<std::pair<Ipv4Address, uint16_t>, TcEtxTrailerBlock> m_retransmitTcEtx;
  ...
  std::map<uint64_t, uint32_t> m_pathSwitchCountByFlow;
   ...
   double GetAdvertisedEdgeEtx(Ipv4Address lastAddr, Ipv4Address destAddr) const;
   void PurgeExpiredAdvertisedEdgeEtx();
   std::optional<TcEtxTrailerBlock> ComputeTcTrailerForQueuedMessage(
       const olsr::MessageHeader& message) const;
   void ApplyTcEtxTrailerForMessage(const olsr::MessageHeader& msg);
 ...
 private:
   ...
   void SendPacket(Ptr<Packet> packet,
                   ...
                   Ptr<Packet> tcEtxTrailer = nullptr);
